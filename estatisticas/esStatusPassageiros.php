...
$portao = carregarPosts('portao'); 
$esteira = carregarPosts('esteira'); 
...
formulario:
$ordenacao = carregarCookie($siglaAeroporto.'_esSPX_ordenacao','st.id, sm.id');
$grupamento = carregarCookie($siglaAeroporto.'_esSPX_grupamento','G-01');
metaTagsBootstrap('');
$titulo = "Estatíticas de Passageiros";
?>
<head>
    ...
    <div class="container alert alert-padrao">
        ...
        <form action="#" method="POST" class="form-group" autocomplete="off">
            <div class="form-group">
                ...
                <input type="hidden" id="hdPortao" <?="value=\"{$portao}\"";?>/>
                <input type="hidden" id="hdEsteira" <?="value=\"{$esteira}\"";?>/>
                ...
                <div class="row pt-2">  
                    <div id="formularioCadastro" class = "col-lg-12 collapse <?="{$collapse}" ?>">  
                        ...
                        <div class="row mt-2">
                            ...
                            <div class="col-md-2">
                                <label for="pslStsPortao">Portão</label>
                                <select class="form-select cpoCookie selCookie input-lg" id="pslStsPortao" name="portao">
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label for="pslStsEsteira">Esteira</label>
                                <select class="form-select cpoCookie selCookie input-lg" id="pslStsEsteira" name="esteira">
                                </select>
                            </div>
                            <div class="col-md-2">
                        ...
                        <div class="row mt-2">
                            <div class="col-md-4">
                                ...
                                <select class="form-select cpoCookie selCookie input-lg" id="pslStsGrupamento" name="grupamento">
                                    ...
                                    <option <?php echo ($grupamento == "G-12") ? 'selected' : '';?> value="G-12">Por portão do voo</option>
                                    <option <?php echo ($grupamento == "G-13") ? 'selected' : '';?> value="G-13">Por esteira do voo</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
<script>
    $(async function() {
        ...
        async function buscarCadastro() {
            var filtro = "";
            var descricaoFiltro = "";
                filtro += " AND st.idAeroporto = "+$("#hdAeroporto").val()+" AND (sm.movimento = 'POU' OR sm.movimento = 'DEC')";
            ...
            $(".cpoCookie").each(function(){
                if (!isEmpty($(this).val())) {
                    switch ($(this).attr('id')) {
                        ...
                        break;
                        case "pslStsPortao":
                            filtro += " AND re.id = "+$("#pslStsPortao").val();
                            descricaoFiltro += ' <br>Portão : '+$("#pslStsPortao :selected").text();
                        break;
                        case "pslStsEsteira":
                            filtro += " AND re.id = '"+$("#pslStsEsteira").val()+"'";
                            descricaoFiltro += ' <br>Esteira : '+$("#pslStsEsteira :selected").text();
                        break;
            ...
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_ordenacao', ordem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_filtro', filtro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_descricao', descricaoFiltro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_grupamento', grupamento);

            await esStatusPassageiros(filtro, ordem, grupamento, descricaoFiltro, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
            $("#pdtStsPeriodoInicio").focus();
        };
        ...
        var pesquisaGrupamento = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPX_grupamento');
        var pesquisaOrdem = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPX_ordenacao');
        var pesquisaFiltro = " " + await valorCookie($('#hdSiglaAeroporto').val()+'_esSPX_filtro');
        var pesquisaDescricao = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPX_descricao');
        ...
        await suCarregarSelectTodos('Movimentos','#pslStsMovimento', $("#hdMovimento").val(), 
            ...
            " AND mo.operacao = 'STA' AND (mo.movimento = 'POU' OR mo.movimento = 'DEC')", 'Consulta');        
        await suCarregarSelectTodos('Recursos','#pslStsPortao', $("#hdPortao").val(),           
            " AND re.idAeroporto = "+$("#hdAeroporto").val()+" AND re.tipo = 'POR'", 'Consulta');
        await suCarregarSelectTodas('Recursos','#pslStsEsteira', $("#hdEsteira").val(),           
            " AND re.idAeroporto = "+$("#hdAeroporto").val()+" AND re.tipo = 'EST'", 'Consulta');            
        ...
        if (isEmpty(pesquisaFiltro)) {
            ...
            pesquisaFiltro = (!isEmpty($("#hdAeroporto").val()) ? " AND st.idAeroporto = "+$("#hdAeroporto").val()+
                                " AND (sm.movimento = 'POU' OR sm.movimento = 'DEC')" : "");
            pesquisaDescricao = (!isEmpty($("#hdAeroporto").val()) ? 'Aeroporto : '+$("#hdNomeAeroporto").val().trim() : ''); 

            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_ordenacao', pesquisaOrdem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_filtro', pesquisaFiltro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_descricao', pesquisaDescricao);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPX_grupamento', pesquisaGrupamento);
        }  
        await esStatusPassageiros(pesquisaFiltro, pesquisaOrdem, pesquisaGrupamento, pesquisaDescricao, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
        $("#pdtStsPeriodoInicio").focus();
    });
</script>