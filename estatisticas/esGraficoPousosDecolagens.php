...
$titulo = 'Pousos e Decolagens - (últimos 3 meses) - '.$siglaAeroporto;

// Pegando o periodo de 90 dias da data atual
$periodoInicio = dateTimeUTC($utcAeroporto)->modify('-4 month')->modify('first day of this month')->format('Y-m-d');  
...
<script>
    $(async function() {
        ...
        const grConfiguracao = (await esConfigurarGrupamentoPousosDecolagens("G-01"));
        const grGrupamento = grConfiguracao[0];

        const filtro = " AND st.idAeroporto = "+$("#hdAeroporto").val()+" AND (sm.movimento = 'POU' OR sm.movimento = 'DEC')"+
        ...
        try {
            var dados = (await esDadosStatusPousosDecolagens(filtro, ordem, grGrupamento)); 
            if (dados != null) {
                await esGraficoStatusPousosDecolagens($("#hdTitulo").val(), dados,'cnvGrafico');
            }
        } catch (error) {
