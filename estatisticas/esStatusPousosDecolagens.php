...
$cabeceira = carregarPosts('cabeceira'); 
...
formulario:
$ordenacao = carregarCookie($siglaAeroporto.'_esSPD_ordenacao','st.id, sm.id');
$grupamento = carregarCookie($siglaAeroporto.'_esSPD_grupamento','G-01');
metaTagsBootstrap('');
$titulo = "Estatíticas de Pousos e Decolagens";
?>
<head>
    ...
    <div class="container alert alert-padrao">
        ...
        <form action="#" method="POST" class="form-group" autocomplete="off">
            <div class="form-group">
                ...
                <input type="hidden" id="hdCabeceira" <?="value=\"{$cabeceira}\"";?>/>
                ...
                <div class="row pt-2">  
                    <div id="formularioCadastro" class = "col-lg-12 collapse <?="{$collapse}" ?>">  
                        ...
                        <div class="row mt-2">
                            ...
                            <div class="col-md-2">
                                <label for="pslStsCabeceira">Cabeceira</label>
                                <select class="form-select cpoCookie selCookie input-lg" id="pslStsCabeceira" name="cabeceira">
                                </select>
                            </div>
                            <div class="col-md-2">
                        ...
                        <div class="row mt-2">
                            <div class="col-md-4">
                                ...
                                <select class="form-select cpoCookie selCookie input-lg" id="pslStsGrupamento" name="grupamento">
                                    ...
                                    <option <?php echo ($grupamento == "G-12") ? 'selected' : '';?> value="G-12">Por cabeceira do voo</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
<script>
    $(async function() {
        ...
        async function buscarCadastro() {
            ...
            $(".cpoCookie").each(function(){
                if (!isEmpty($(this).val())) {
                    switch ($(this).attr('id')) {
                        ...
                        break;
                        case "pslStsCabeceira":
                            filtro += " AND re.id = "+$("#pslStsCabeceira").val();
                            descricaoFiltro += ' <br>Cabeceira : '+$("#pslStsCabeceira :selected").text();
                        break;
            ...
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_ordenacao', ordem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_filtro', filtro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_descricao', descricaoFiltro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_grupamento', grupamento);

            await esStatusPousosDecolagens(filtro, ordem, grupamento, descricaoFiltro, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
            $("#pdtStsPeriodoInicio").focus();
        };
        ...
        var pesquisaGrupamento = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPD_grupamento');
        var pesquisaOrdem = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPD_ordenacao');
        var pesquisaFiltro = " " + await valorCookie($('#hdSiglaAeroporto').val()+'_esSPD_filtro');
        var pesquisaDescricao = await valorCookie($('#hdSiglaAeroporto').val()+'_esSPD_descricao');
        ...
        await suCarregarSelectTodos('Recursos','#pslStsCabeceira', $("#hdCabeceira").val(),           
            " AND re.idAeroporto = "+$("#hdAeroporto").val()+" AND re.tipo = 'PIS'", 'Consulta');
        ...
        if (isEmpty(pesquisaFiltro)) {
            ...
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_ordenacao', pesquisaOrdem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_filtro', pesquisaFiltro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_descricao', pesquisaDescricao);
            await criarCookie($('#hdSiglaAeroporto').val()+'_esSPD_grupamento', pesquisaGrupamento);
        }  
        await esStatusPousosDecolagens(pesquisaFiltro, pesquisaOrdem, pesquisaGrupamento, pesquisaDescricao, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
        $("#pdtStsPeriodoInicio").focus();
    });
</script>