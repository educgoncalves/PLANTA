...
$titulo = 'Passageiros - (últimos 3 meses) - '.$siglaAeroporto;
...
<script>
    $(async function() {
        ...
        const grConfiguracao = (await esConfigurarGrupamentoPassageiros("G-01"));
        ...
        try {
            var dados = (await esDadosStatusPassageiros(filtro, ordem, grGrupamento)); 
            if (dados != null) {
                await esGraficoStatusPassageiros($("#hdTitulo").val(), dados,'cnvGrafico');
            }
        } catch (error) {
