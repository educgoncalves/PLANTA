...
$aeroporto = carregarGets('site');
if ($aeroporto != '') {
    // Desativar todas as conexões do aeroporto atual + sistema + usuario + identificacao
    $_retorno = desativarConexao($_SESSION['plantaIDSite'], $_SESSION['plantaSistema'], $_SESSION['plantaUsuario'], $_SESSION['plantaIPCliente']);
    if ($_retorno['status'] == "OK"){
        $dados = explode("|", $aeroporto);
        $_SESSION['plantaSite'] = $dados[0];
        $_SESSION['plantaGrupo'] = $dados[1];
        $_SESSION['plantaNivel'] = $dados[2];
        $_SESSION['plantaIDSite'] = $dados[3];
        $_SESSION['plantaNomeSite'] = $dados[4];
        $_SESSION['plantaSistema'] = $dados[5];
        $_SESSION['plantaLocalidadeSite']  = $dados[6];
