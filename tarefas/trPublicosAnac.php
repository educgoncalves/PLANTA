...
function executarImportacaoPublicosAnac($identificacao, $usuario = 'GEAR', $modo = 'AUT') {
    ...
    $tarefa = 'APUB';
    $resultado = "trPublicosAnac_".$identificacao;
    $desprezadas = "trPublicosAnac_Desprezadas_".$identificacao;
    $_mensagens[] = "";
    $_mensagens[] = "Importação de Aeródromos Públicos da ANAC - executado por ".$usuario;
    $_mensagens[] = "";
    ...
    try {
        ...
        // Verificando a primeira linha do arquivo
                 if ($linhasTotal == 1) {
                    ...
                    if (strpos($dados[0], "Atualizado") === FALSE) {
                        throw new PDOException("Arquivo não é o esperado para a importação dos aeródromos públicos ANAC!");
                    }
                } 
        ...
        // Se o arquivo é válido, inicia o controle de atualização
                if ($linhasTotal == 2) {
                    ...
                    if (strpos($linha,"Código OACI;CIAD;Nome;Município") === FALSE) {
                        throw new PDOException("Arquivo não é o esperado para a importação dos aeródromos públicos ANAC! [".$linha."]");
                    } else {
                        // Marcar registros de origem = PUB (importação) com origem = ATU (atualizando) 
                        $comando = "UPDATE gear_aeroportos SET origem = 'ATU' WHERE origem = 'PUB'";
                        $sql = $conexao->prepare($comando); 
                        if (!$sql->execute()) {
                            throw new PDOException("Não consegui preparar os aeródromos públicos para atualização!");
                        }
                    }
                }
            ...
            // Atualizando AEROPORTOS PUBLICOS que foram importados por esta importação 
                ...
                // Só regravar se fonte ANAC e origem ATU
                                if ($fonte == 'ANAC' && $origem == 'ATU') {
                                    $comando = "UPDATE gear_aeroportos SET nome = '".$nome."', localidade = '".$localidade."', pais = '".$pais."', origem = 'PUB', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;                         
                                } else {
                                    $linhasDesprezadas++;
                                    gravaXLogProcesso($desprezadas, "warning", $icao. ' [Aeroporto importado por outro processo]', $identificacao);
                                }
                            } else {
                                $comando = "INSERT INTO gear_aeroportos(icao, nome, localidade, pais, fonte, origem, cadastro) VALUES('".
                                            $icao."','".$nome."','".$localidade."','".$pais."','ANAC', 'PUB', UTC_TIMESTAMP())";
                            } 
                            if ($comando != '') {
                                $sql = $conexao->prepare($comando); 
                                if (!$sql->execute()){
        ...
        // Senão, finaliza o controle de atualização
            if ($linhasTotal == 0) {
                ...
                $sql = $conexao->prepare($comando); 
                if (!$sql->execute()){ 
                    $erros[] = "Não foi possível desativar os aeródromos públicos ANAC pendentes!";
                }
           }   
        } else {
