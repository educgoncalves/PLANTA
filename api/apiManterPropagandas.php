...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
            case 'Alterar':
                $_chaves = array('aeroporto','empresa','propaganda','dtInicio','dtFinal','situacao');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                       throw new PDOException("API ".$_api." 003 - [".$_funcao."] - Falta parâmetro [".$_chave."] obrigatório para executar!");
                    }
                ...
                $empresa = $_dados['empresa'];
                $propaganda = $_dados['propaganda'];
                $dtInicio = $_dados['dtInicio'];
                $dtFinal = $_dados['dtFinal'];
...
// ********************************************************************
    if ($_funcao == 'Incluir' || $_funcao == 'Alterar') {
        try {
            $dataInicio = mudarDataAMD($dtInicio);
            $dataFinal = mudarDataAMD($dtFinal);
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_propagandas SET idAeroporto = ".$aeroporto.", empresa = '".$empresa.
                            "', propaganda = '".$propaganda."', dtInicio = ".$dataInicio.", dtFinal = ".$dataFinal.
                            ", situacao = '".$situacao.
                            "', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_propagandas (idAeroporto, empresa, propaganda, dtInicio, dtFinal, situacao, cadastro) VALUES (".
                            $aeroporto.", '".$empresa."', '".$propaganda."', ".$dataInicio.", ".$dataFinal.", '".
                            $situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_propagandas", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_propagandas WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_propagandas", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
