...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
            case 'Alterar':
                $_chaves = array('aeroporto','tipo','numero','item','situacao');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                        throw new Exception("API - Faltam parâmetros para executar esta Y função!");
                    }
                }
            ...
            case 'Despadronizar':
            case 'Copiar':
                $_chaves = array('aeroporto');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                        throw new Exception("API - Faltam parâmetros para executar esta X função!");
                    }
                }
                $aeroporto = $_dados['aeroporto'];
            break;
...
// ********************************************************************
    if ($_funcao == 'Incluir' || $_funcao == 'Alterar') {
        try {
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_vistoria_itens SET tipo = '".$tipo."', numero = '".$numero."', idAeroporto = "
                            .$aeroporto.", item = '".$item."', situacao = '".$situacao."', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_vistoria_itens (tipo, numero, idAeroporto, item, situacao, cadastro) VALUES ('".
                            $tipo."', '".$numero."', ".$aeroporto.", '".$item."', '".$situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_vistoria_itens", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_vistoria_itens WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_vistoria_itens", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
...
// ********************************************************************
    if ($_funcao == "Copiar") {
        try {
            $conexao = conexao();
            $comando = "INSERT INTO gear_vistoria_itens (tipo, numero, idAeroporto, item, situacao, cadastro)
                        SELECT tipo, numero, ".$aeroporto.", item, situacao, UTC_TIMESTAMP() FROM gear_vistoria_itens WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_vistoria_itens", "Copiar", $siglaAeroporto, $usuario, $id, $comando);   
...
// FUNÇÃO: Padronizar
// ********************************************************************
    if ($_funcao == "Padronizar") {
        try {
            $conexao = conexao();
            $comando = "UPDATE gear_vistoria_itens SET idAeroporto = 0 WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_vistoria_itens", "Padronizar", $siglaAeroporto, $usuario, $id, $comando);   
                $_retorno = ['status' => 'OK', 'msg'=> "Registro padronizar com sucesso!"];
            } else {
                throw new PDOException("Não foi possível padronizar este registro!");
            } 
        } catch (PDOException $e) {
            gravaTrace(traduzPDO($e->getMessage())."\n".$_comando);
            $_retorno = ['status' => 'ERRO', 'msg' => traduzPDO($e->getMessage())];
...
// ********************************************************************
// FUNÇÃO: Despadronizar
// ********************************************************************
    if ($_funcao == "Despadronizar") {
        try {
            $conexao = conexao();
            $comando = "UPDATE gear_vistoria_itens SET idAeroporto = ".$aeroporto." WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_vistoria_itens", "Despadronizar", $siglaAeroporto, $usuario, $id, $comando);   
                $_retorno = ['status' => 'OK', 'msg'=> "Registro despadronizar com sucesso!"];
            } else {
                throw new PDOException("Não foi possível despadronizar este registro!");
            } 
        } catch (PDOException $e) {
            gravaTrace(traduzPDO($e->getMessage())."\n".$_comando);
            $_retorno = ['status' => 'ERRO', 'msg' => traduzPDO($e->getMessage())];
