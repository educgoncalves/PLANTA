...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
            case 'Alterar':
                $_chaves = array('aeroporto','localizacao','situacao');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                       throw new PDOException("API ".$_api." 003 - [".$_funcao."] - Falta parâmetro [".$_chave."] obrigatório para executar!");
                    }
                ...
                $localizacao = $_dados['localizacao'];
...
// ********************************************************************
    if ($_funcao == 'Incluir' || $_funcao == 'Alterar') {
        try {
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_monitores SET idAeroporto = ".$aeroporto.", localizacao = '".$localizacao.
                            "', situacao = '".$situacao."', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_monitores (idAeroporto, localizacao, situacao, cadastro) VALUES (".
                            $aeroporto.", '".$localizacao."', '".$situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_monitores", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_monitores WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_monitores", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
