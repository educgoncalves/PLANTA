...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
            case 'Alterar':
                $_chaves = array('codigoAnac','nome','telefone','email','situacao');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                       throw new PDOException("API ".$_api." 003 - [".$_funcao."] - Falta parâmetro [".$_chave."] obrigatório para executar!");
                    }
                }
                $codigoAnac = $_dados['codigoAnac'];
                $nome = $_dados['nome'];
...
// ********************************************************************
    if ($_funcao == 'Incluir' || $_funcao == 'Alterar') {
        try {
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_comandantes SET codigoAnac = '".$codigoAnac."', nome = '".$nome."', telefone = '".$telefone.
                            "', email= '".$email."', situacao = '".$situacao."', cadastro  = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_comandantes (codigoAnac, nome, telefone, email, situacao, cadastro) VALUES ('".
                            $codigoAnac."', '".$nome."', '".$telefone."', '".$email."', '".$situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_comandantes", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_comandantes WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_comandantes", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
