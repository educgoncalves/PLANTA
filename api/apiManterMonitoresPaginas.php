...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
            case 'Alterar':                
                $_chaves = array('idMonitor','pagina','acao','segundos','resolucao','situacao');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                       throw new PDOException("API ".$_api." 003 - [".$_funcao."] - Falta parâmetro [".$_chave."] obrigatório para executar!");
                    }
                }
                $idMonitor = $_dados['idMonitor'];
                $pagina = $_dados['pagina'];
                $acao = $_dados['acao'];
                $segundos = $_dados['segundos'];
                $resolucao = $_dados['resolucao'];
...
// ********************************************************************
    if ($_funcao == 'Incluir' || $_funcao == 'Alterar') {
        try {
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_monitores_paginas SET pagina = '".$pagina."', acao = '".$acao.
                            "', segundos = ".$segundos.", resolucao = '".$resolucao."', situacao = '".$situacao.
                            "', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_monitores_paginas (idMonitor, pagina, acao, segundos, resolucao, situacao, cadastro) VALUES (".
                            $idMonitor.", '".$pagina."', '".$acao."', ".$segundos.", '".$resolucao."', '".$situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_monitores_paginas", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_monitores_paginas WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_monitores_paginas", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
