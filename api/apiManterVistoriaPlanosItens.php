...
// ********************************************************************
    ...
    try {
        ...
        switch ($_funcao) {
            case 'Incluir':
                $_chaves = array('idPlano','idItem');
                foreach($_chaves as $_chave){
                    if (!array_key_exists($_chave, $_dados)) {
                        throw new Exception("API ".$_api." [".$_funcao."] - Faltam parâmetros para executar esta função!");
                    }
                }
                $idPlano = $_dados['idPlano'];
                $idItem = $_dados['idItem'];
            break;
...
// ********************************************************************
    if ($_funcao == 'Incluir') {
        try {
            $conexao = conexao();
            $comando = "INSERT INTO gear_vistoria_planos_itens (idPlano, idItem, cadastro) VALUES (".
                        $idPlano.", ".$idItem.", UTC_TIMESTAMP())";

            $sql = $conexao->prepare($comando); 
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLogAPI("gear_vistoria_planos_itens", ($id != "" ? "Alteração" : "Inclusão"), $siglaAeroporto, $usuario, 
...
// ********************************************************************
    if ($_funcao == "Excluir") {
        try {
            $conexao = conexao();
            $comando = "DELETE FROM gear_vistoria_planos_itens WHERE id = ".$id;
            $sql = $conexao->prepare($comando); 
            if ($sql->execute()){
                gravaDLogAPI("gear_vistoria_planos_itens", "Exclusão", $siglaAeroporto, $usuario, $id, $comando);   
