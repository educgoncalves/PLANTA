...
formulario:
$ordenacao = carregarCookie($siglaAeroporto.'_faRM_ordenacao','ae.icao,rm.ano,rm.numero');            
metaTagsBootstrap('');
$titulo = "Remessas";
?>
<head>
                ...
                <div class="row mt-2">                      
                    <div class="col-md-8">
                        ...
                        <select class="form-select cpoCookie selCookie input-lg" id="pslOrdenacao">
                            <option <?php echo ($ordenacao == 'ae.icao,rm.ano,rm.numero') ? 'selected' : '';?> value='ae.icao,rm.ano,rm.numero'>Remessa</option>
                            <option <?php echo ($ordenacao == 'rm.cadastro,ae.icao,rm.ano,rm.numero') ? 'selected' : '';?> value='rm.cadastro,ae.icao,rm.ano,rm.numero'>Data</option>
                        </select> 
                    </div>
                </div>                
            </div>
...
<script>
    $(async function() {
        ...
        async function buscarCadastro() {
            var filtro = "";
            var descricaoFiltro = "";
            filtro += (!isEmpty($("#hdAeroporto").val()) ? " AND rm.idAeroporto = "+$("#hdAeroporto").val() : "");
            ...
            $(".cpoCookie").each(function(){
                if (!isEmpty($(this).val())) {
                    switch ($(this).attr('id')) {
                        case "pslSituacao":
                            filtro += " AND rm.situacao = '"+$("#pslSituacao").val()+"'";
            ...
            await criarCookie($('#hdSiglaAeroporto').val()+'_faRM_ordenacao', ordem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_faRM_filtro', filtro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_faRM_descricao', descricaoFiltro);
            
            await faCarregarRemessas('Consultar', filtro, ordem, descricaoFiltro, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
        }
        ...
        var pesquisaOrdem = await valorCookie($('#hdSiglaAeroporto').val()+'_faRM_ordenacao');
        var pesquisaFiltro = " " + await valorCookie($('#hdSiglaAeroporto').val()+'_faRM_filtro');
        var pesquisaDescricao = await valorCookie($('#hdSiglaAeroporto').val()+'_faRM_descricao');
        ...
        if (isEmpty(pesquisaFiltro)) {
            pesquisaFiltro = (!isEmpty($("#hdAeroporto").val()) ? " AND rm.idAeroporto = "+$("#hdAeroporto").val() : "");
            pesquisaDescricao = (!isEmpty($("#hdAeroporto").val()) ? ' <br>Aeroporto : '+$("#hdNomeAeroporto").val(): '');
        }        
        await faCarregarRemessas('Consultar', pesquisaFiltro, pesquisaOrdem, pesquisaDescricao, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
    });
</script>