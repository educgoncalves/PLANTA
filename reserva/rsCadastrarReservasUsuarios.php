...
if ($evento == "salvar") {
    ...
    $erros = camposPreenchidos(['usuario','nome','email']); //,'situacao']);
    if (!$erros) {
        try {
            $conexao = conexao();
            if ($id != "") {
                $comando = "UPDATE gear_reservas_usuarios SET usuario = '".$usuario."', nome = '".$nome."', email = '".$email."', situacao = '".
                            $situacao."', cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
            } else {
                $comando = "INSERT INTO gear_reservas_usuarios (usuario, senha, nome, email, fonte, situacao, cadastro) VALUES ('".
                            $usuario."', sha1('". $usuario."'), '".$nome."', '".$email."', 'GEAR', '".$situacao."', UTC_TIMESTAMP())";
            }
            $sql = $conexao->prepare($comando);               
            if ($sql->execute()) {
                if ($sql->rowCount() > 0) {
                    gravaDLog("gear_reservas_usuarios", ($id != "" ? "Alteração" : "Inclusão"), $_SESSION['plantaAeroporto'], 
...
if ($evento == "reset" && $id != "") {
    try {
        $conexao = conexao();
        $comando = "UPDATE gear_reservas_usuarios SET senha = sha1(usuario), cadastro = UTC_TIMESTAMP() WHERE id = ".$id;
        $sql = $conexao->prepare($comando);               
        if ($sql->execute()) {
            if ($sql->rowCount() > 0) {
                gravaDLog("gear_reservas_usuarios", "Reset", $_SESSION['plantaAeroporto'], $_SESSION['plantaUsuario'], $id, $comando);            
...
if ($evento == "recuperar" && $id != "") {
    try {
        $conexao = conexao();
        $comando = "SELECT * FROM gear_reservas_usuarios WHERE id = ".$id;
        $sql = $conexao->prepare($comando);               
        if ($sql->execute()) {
...
if ($evento == "excluir" && $id != "") {
    try {
        $conexao = conexao();
        $comando = "DELETE FROM gear_reservas_usuarios WHERE id = ".$id;
        $sql = $conexao->prepare($comando);               
        if ($sql->execute()){
            gravaDLog("gear_reservas_usuarios", "Exclusão", $_SESSION['plantaAeroporto'], $_SESSION['plantaUsuario'], $id, $comando);            
...
formulario:
$ordenacao = carregarCookie($siglaAeroporto.'_opCU_ordenacao','us.usuario');     
metaTagsBootstrap('');
$titulo = "Reservas Usuários";
?>
<head>
                    ...
                    <div id="formularioCadastro" class = "col-lg-12 collapse <?="{$collapse}" ?>">     
                        <div class="row mt-2">
                            <div class="col-md-2">
                                ...
                                <input type="text" class="form-control cpoObrigatorio cpoLimpar input-lg" id="txUsuario" name="usuario" maxlength="13"
...
<script>
    $(async function() {
        ...
        async function buscarCadastro() {
            ...
            $(".cpoCookie").each(function(){
                if (!isEmpty($(this).val())) {
                    switch ($(this).attr('id')) {
                        case "ptxUsuario":
                            filtro += " AND us.usuario LIKE '%"+$("#ptxUsuario").val()+"%'";
            ...
            await criarCookie($('#hdSiglaAeroporto').val()+'_opCU_ordenacao', ordem);
            await criarCookie($('#hdSiglaAeroporto').val()+'_opCU_filtro', filtro);
            await criarCookie($('#hdSiglaAeroporto').val()+'_opCU_descricao', descricaoFiltro);
                                    
            await rsCarregarReservasUsuarios('Cadastrar', filtro, ordem, descricaoFiltro, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
            $("#txUsuario").focus();
        }
        ...
        var pesquisaOrdem = await valorCookie($('#hdSiglaAeroporto').val()+'_opCU_ordenacao');
        var pesquisaFiltro = " " + await valorCookie($('#hdSiglaAeroporto').val()+'_opCU_filtro'); 
        var pesquisaDescricao = await valorCookie($('#hdSiglaAeroporto').val()+'_opCU_descricao');
        ...
        $("#ptxUsuario").mask('Y', {'translation': {Y: {pattern: /[0-9]/,recursive: true},}});

        // Adequações para o cadastro  
        await suCarregarSelectTodas('TodosSituacao','#slSituacao', $('#hdSituacao').val(),'','Cadastrar');
        await rsCarregarReservasUsuarios('Cadastrar', pesquisaFiltro, pesquisaOrdem, pesquisaDescricao, parseInt($('#hdPagina').val()), parseInt($('#hdLimite').val()));
        $("#txUsuario").focus();
        $("#txUsuario").mask('Y', {'translation': {Y: {pattern: /[0-9]/,recursive: true},}});
    });
</script>